...
def main(arguments):
    ...
    if arguments['--plugin']:
        ...
        pac_path = os.path.join(arguments['PATH_TO_BRO_SRC'], "src/analyzer/protocol", name.lower())
        script_path = os.path.join(arguments['PATH_TO_BRO_SRC'], "scripts/base/protocols", name.lower())
        do_plugin = False
        mkdir(pac_path)
        mkdir(script_path)
