##! Implements base functionality for {{ name }} analysis.
##! Generates the {{ name|title }}.log file.
...
event bro_init() &priority=5
	{
	Log::create_stream({{ name|title }}::LOG, [$columns=Info, $ev=log_{{ name|lower }}, $path="{{ name|lower }}"]);
